  ...
  export const queryUserWorkspaceAction: Action = {
    ...
    handler: async (
      ...
      try {
        ...
        elizaLogger.debug("Query workspace context:", queryWorkspaceContext);
        ...
        elizaLogger.debug("Full generated object:", JSON.stringify(generatedResult));
        ...
        const content = generatedResult.object
          ...
          : generatedResult;
        elizaLogger.debug("Unwrapped generated content:", JSON.stringify(content));
        ...
        elizaLogger.debug("Querying workspace for address:", content.workspace);
        const workspace = await queryWorkspaceByAddress(content.workspace);
        elizaLogger.debug("Workspace retrieved:", JSON.stringify(workspace));
        if (!workspace) {
        ...
        const keys = await queryKeysByWorkspace(workspace.workspace.address);
        elizaLogger.debug("Queried keys:", JSON.stringify(keys));
  
        // Format the response.
        let responseText = `🏢 Workspace Details:\n
        - Address: ${workspace.workspace.address}
        - Owners:\n ${workspace.workspace.owners.map((owner, index) => `${index + 1}. ${owner}`).join("\n")}
        ...
        if (keys && keys.length > 0) {
          responseText += keys
            .map(
              (key, index) =>
                `${index + 1}. KeyId: ${key.key.id} of type ${key.key.type} \n ${key.wallets
                  .map((wallet: any, index: number) => `${index + 1}. ${wallet.address}`)
                  .join("\n")}`
            )
            .join("\n\n");
        } else {
